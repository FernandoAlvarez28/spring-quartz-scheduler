--This script is executed on startup by using "INIT=RUNSCRIPT FROM 'classpath:scripts/init.sql'" in the "spring.datasource.url" property.
--But for some reason is executed again when Quartz connects to the Web application's datasource (which runs this script)
--to schedule a Job, so the "if not exists" clauses become necessary.
